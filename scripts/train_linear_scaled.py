"""Run a mini-batch scaled training experiment on synthetic linear panels."""
...
def main() -> None:
    ...
    data = pd.read_csv(root / "data" / "testdata" / "panels" / "linear_panel.csv")
    ...
    naive_predictions = inverse_standardize(X[:, -1, :], metadata, scaling_params, ["panel"])
    ...
    naive_metrics = _metrics(original_targets, naive_predictions)
    summary_lines = [
        "Scaled linear panel training summary",
        f"device: {device}",
        f"windows: {len(X)}",
        ...
        *(f"  {name}: {value:.10f}" for name, value in original_metrics.items()),
        "naive_original_metrics:",
        *(f"  {name}: {value:.10f}" for name, value in naive_metrics.items()),
        "sample_predictions:",
    ]
    sample_rows = []
    for index in range(min(5, len(metadata))):
        ...
        sample = {
            ...
            "target_standardized": float(standardized_targets[index, 0]),
            "prediction_standardized": float(standardized_predictions[index, 0]),
    ...
    output_path = root / "output" / "train_linear_scaled_summary.txt"
    ...
    print(f"naive original RMSE/MAE: {naive_metrics['rmse']:.6f}/{naive_metrics['mae']:.6f}")
